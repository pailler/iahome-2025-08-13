...
async function generateTokenViaAPI() {
  ...
  try {
    console.log(`🔍 Génération de token via API pour: ${targetEmail}`);
    ...
    // 3. Générer un token via l'API
    console.log('🔑 Génération du token via API...');
    
    const response = await fetch('http://localhost:8021/api/generate-module-token', {
      method: 'POST',
      headers: {
    ...
    if (result.success) {
      console.log('✅ Token généré avec succès via API !');
      console.log('\n📋 Détails du token:');
